...
const QuestionGrid = ({ questions, selectedAnswers, currentIndex, onSelect, onClose }) => (
...
const ReviewCard = ({ answer, index, options }) => {
  const selectedLabel = answer.selectedOption
    ? String.fromCharCode(65 + ['A','B','C','D'].indexOf(answer.selectedOption))
    : null;
  const correctLabel = String.fromCharCode(65 + ['A','B','C','D'].indexOf(answer.correctOption));
  const selectedText = answer.selectedOption && options ? options[['A','B','C','D'].indexOf(answer.selectedOption)] : null;
  const correctText = options ? options[['A','B','C','D'].indexOf(answer.correctOption)] : null;

  return (
    <div className={`bg-[color:var(--bg-card)] border rounded-xl p-5 ${answer.isCorrect ? 'border-[color:var(--success)]' : 'border-[color:var(--danger)]'}`}>
      ...
      {!answer.isCorrect && selectedText && (
        <div className="flex items-center gap-2 py-2 px-3 bg-[color:var(--danger-light)] rounded-lg mb-2 text-sm text-[color:var(--danger)]">
          ...
          <span>Your answer: <strong>{selectedLabel}. {selectedText}</strong></span>
        </div>
      )}
      ...
      <div className="flex items-center gap-2 py-2 px-3 bg-[color:var(--success-light)] rounded-lg text-sm text-[color:var(--success)]">
        ...
        <span>Correct answer: <strong>{correctLabel}. {correctText}</strong></span>
      </div>
    </div>
  );
};
...
const QuizData = () => {
  ...
  const handleSubmitQuiz = async () => {
    ...
    Object.entries(selectedAnswers).forEach(([questionId, selectedOption]) => {
      ...
      if (question) {
        ...
        if (index !== -1) formattedAnswers[questionId] = String.fromCharCode(65 + index);
      }
    });
    ...
    try {
      ...
      setShowResults(true);
      toast[timeLeft === 0 ? 'info' : 'success'](timeLeft === 0 ? "Time's up! Quiz submitted." : "Quiz completed! 🎉");
    } catch (error) {
  ...
  if (showReview && resultData?.answers) {
    const filteredAnswers = resultData.answers.filter(a =>
      reviewFilter === 'all' ? true : reviewFilter === 'correct' ? a.isCorrect : !a.isCorrect
    );
    const correctCount = resultData.answers.filter(a => a.isCorrect).length;
    const wrongCount = resultData.answers.filter(a => !a.isCorrect).length;
          ...
          <div className="flex-1 min-w-0">
            ...
            {currentQuestion && (
              <div className="bg-[color:var(--bg-card)] border border-[color:var(--accent-subtle)] rounded-xl p-7 mb-5 max-sm:p-5">
                <span className="inline-block py-1 px-3 bg-[color:var(--accent-light)] text-[color:var(--accent)] rounded-full text-[11px] font-semibold mb-3.5">Q{currentQuestionIndex + 1}</span>
